...
A more conservative (and appropriate) approach  for reporting these results would be to employ some non-parametric approaches. These statistical tests would not be adversely affected by the issues that plague our *t*-test and would lead to more robust inference. We'll discuss and demonstrate a few approaches below. 
...
The non-parametric alternative to the two independent samples t-test is the Wilcoxon rank-sum test (sometimes called the Mann-Whitney U-Test). 
...
We have seen permutation tests earlier in this course. The idea is that we create a distribution of all possible t-statistics that could be obtained by shuffling the group labels in our data set. The number of observed test statistics that are larger than our observed test statistic is taken to be our p-value. 

Permutation tests are assumptionless and are a powerful tool for the modern statistician. The following code shows how to perform a permutation test for this example. 
...
THe following two sections include a link to a data set with an accompanying story. Each example shows includes a comparison of two quantitative variables. Please download the data sets, decide which type of 2 sample test is most appropriate, and follow the 5-step hypothesis testing approach discussed in class. 
